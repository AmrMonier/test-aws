...
import { sendResetPasswordMail, confirmPasswordResetedMail } from "../helpers/sendGrid.js";
  ...
  async resetPassword(req, res, next) {
    super.reportErrors(req, res, next).then(() => {
      ...
      Admin.findOne({ email }).then(async (admin) => {
        ...
        Token.create({ token, owner: admin._id, type: 'reset' }).then((token) => {
  ...
  async update(req, res, next) {
    super
      ...
      .then(() => {
        ...
        const { name } = req.body;
        Admin.findByIdAndUpdate(id, { name }, { new: true }).then(
          (admin) => {
            if (!admin) {
